  ...
  const nav_msgs::msg::Odometry::ConstSharedPtr msg)
{
  ...
  m_current_velocity_ptr->stamp = msg->header.stamp;
  m_current_velocity_ptr->velocity_mps = static_cast<float>(msg->twist.twist.linear.x);
  m_current_velocity_ptr->rear_wheel_angle_rad = static_cast<float>(msg->twist.twist.angular.z);
}
...
void LongitudinalController::callbackTimerControl()
{
  ...
  if (!m_current_velocity_ptr || !m_prev_velocity_ptr || !m_trajectory_ptr) {
    return;
  }

  // wait for current ego pose
  geometry_msgs::msg::TransformStamped transform;
  try {
    transform = m_tf_buffer.lookupTransform(
      m_trajectory_ptr->header.frame_id, "base_link", tf2::TimePointZero);
...
{
  const float64_t dv = m_current_velocity_ptr->velocity_mps - m_prev_velocity_ptr->velocity_mps;
  const float64_t dt = std::max(
    (rclcpp::Time(m_current_velocity_ptr->stamp) - rclcpp::Time(m_prev_velocity_ptr->stamp))
      .seconds(),
    1e-03);
