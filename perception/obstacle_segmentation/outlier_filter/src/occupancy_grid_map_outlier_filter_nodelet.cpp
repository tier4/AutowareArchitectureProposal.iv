...
#include "outlier_filter/occupancy_grid_map_outlier_filter_nodelet.hpp"
  ...
  const std::string & target_frame, sensor_msgs::msg::PointCloud2 & output)
{
  ...
  try {
    ...
    RCLCPP_WARN_THROTTLE(
      rclcpp::get_logger("outlier_filter").get_child("occupancy_grid_map_outlier_filter"), clock,
      5000, "%s", ex.what());
    return false;
  }
...
RCLCPP_COMPONENTS_REGISTER_NODE(outlier_filter::OccupancyGridMapOutlierFilterComponent)
