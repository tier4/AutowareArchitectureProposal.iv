...
#include "ground_filter/ransac_ground_filter_nodelet.hpp"
...
ground_filter::PlaneBasis getPlaneBasis(const Eigen::Vector3d & plane_normal)
{
  ground_filter::PlaneBasis basis;
  ...
  const pcl::PointCloud<PointType> segment_ground_cloud, const Eigen::Vector3d & plane_normal)
{
  ...
  const ground_filter::PlaneBasis basis = getPlaneBasis(plane_normal);
  Eigen::Matrix3d rot;
...
RCLCPP_COMPONENTS_REGISTER_NODE(ground_filter::RANSACGroundFilterComponent)
