...
Graph *init_from_cost_mat(const std::vector<std::vector<double>> &cost)
{
    ...
    auto add_edge_wrapper = [&](int tail, int head, double weight) { resG->add_edge(tail - 1, head - 1, edge_id++, weight); };
    ...
    for (int j = 1; j <= n_cols; j++)
    {
        ...
        add_edge_wrapper(1, (j + n_rows) * 2, 100);
    }
...
void convert_flow_to_assignment(Graph *resG, std::vector<std::vector<int>> path_set,
                                int n_rows, int n_cols,
                                std::unordered_map<int, int> *direct_assignment,
                                std::unordered_map<int, int> *reverse_assignment)
// std::vector<std::vector<bool>> *assignment_mat)
{
    // int n_rows = assignment_mat->size();
    // int n_cols = assignment_mat->at(0).size();
    // int n_rows = direct_assignment->size();
    // int n_cols = reverse_assignment->at(0).size();
    ...
    auto is_bridge = [&](int a, int b) { return (a > resG->src_id_ && a <= n_rows * 2 && a % 2 == 0) && (b > n_rows * 2 && b < resG->sink_id_ && b % 2 == 1); };
    ...
    for (size_t i = 0; i < path_set.size(); i++)
    {
        for (size_t j = 0; j < path_set[i].size() - 1; j++)
        {
            ...
            // // Check if head and tail are included in {h_i, o_j}
            // if (is_bridge(head, tail))
            // {
            //     // head is h_i, and tail is o_j
            //     //pass
            // }
            // else if (is_bridge(tail, head))
            // {
            //     // tail is h_i, and head is o_j
            //     std::swap(head, tail);
            // }
            // else
            // {
            ...
            // int i_row = head / 2 - 1;
            // int i_col = (tail + 1) / 2 - n_rows - 1;
            // assert(i_row >= 0 && i_row <= n_rows - 1);
            // assert(i_col >= 0 && i_col <= n_cols - 1);
            // assignment_mat->at(i_row).at(i_col) = !assignment_mat->at(i_row).at(i_col);
...
// int solve_muSSP(std::string inFileName,
int solve_muSSP(const std::vector<std::vector<double>> &cost,
                std::unordered_map<int, int> *direct_assignment,
                std::unordered_map<int, int> *reverse_assignment)
// std::vector<std::vector<bool>> *assignment_mat)
{
    ...
    // std::chrono::system_clock::time_point start_time_muSSP, end_time_muSSP;
    // start_time_muSSP = std::chrono::system_clock::now();
    ...
    // Get assignments
    convert_flow_to_assignment(org_graph.get(), path_set, cost.size(), cost.at(0).size(), direct_assignment, reverse_assignment);

    // end_time_muSSP = std::chrono::system_clock::now();
    // double time_muSSP = static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(end_time_muSSP - start_time_muSSP).count() / 1000.0);
    // std::cout << std::endl << time_muSSP << std::endl;
