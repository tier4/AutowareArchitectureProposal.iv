  ...
  tf_listener_ptr_(tf_buffer_)
{
  ...
  for (int id = 0; id < rois_number_; ++id) {
    ...
    v_camera_info_sub_.push_back(
      this->create_subscription<sensor_msgs::msg::CameraInfo>(
        "camera_info" + std::to_string(id), 1, fcn));
  }
  ...
  for (int id = 0; id < static_cast<int>(v_roi_sub_.size()); ++id) {
    v_roi_sub_.at(id) = std::make_shared<
      ...
      this, "rois" + std::to_string(id), rclcpp::QoS{1}.get_rmw_qos_profile());
  }
  ...
  autoware_perception_msgs::msg::DynamicObjectWithFeatureArray::ConstSharedPtr input_roi7_msg)
{
  ...
  for (int id = 0; id < static_cast<int>(v_roi_sub_.size()); ++id) {
    ...
    cv::Mat image(cv::Size(
        m_camera_info_.at(id).width, m_camera_info_.at(
          id).height), CV_8UC3, cv::Scalar(0, 0, 0));
    ...
    for (size_t i = 0; i < input_cluster_msg->feature_objects.size(); ++i) {
      ...
      tf2::doTransform(
        input_cluster_msg->feature_objects.at(
          i).feature.cluster, transformed_cluster, transform_stamped);
        ...
        ++iter_x, ++iter_y, ++iter_z)
      {
          ...
          static_cast<int>(m_camera_info_.at(id).height) - 1)
        {
          ...
          cv::circle(
            ...
            cv::Point(
              static_cast<int>(normalized_projected_point.x()),
              static_cast<int>(normalized_projected_point.y())), 2, cv::Scalar(255, 255, 255), 3,
            4);
        }
      }
      if (projected_points.empty()) {
    ...
    for (size_t i = 0; i < input_roi_msg->feature_objects.size(); ++i) {
      cv::line(
        image,
        cv::Point(
          input_roi_msg->feature_objects.at(i).feature.roi.x_offset,
          input_roi_msg->feature_objects.at(i).feature.roi.y_offset),
        cv::Point(
          input_roi_msg->feature_objects.at(i).feature.roi.x_offset +
          input_roi_msg->feature_objects.at(i).feature.roi.width,
          input_roi_msg->feature_objects.at(i).feature.roi.y_offset),
      ...
      cv::line(
        ...
        cv::Point(
          input_roi_msg->feature_objects.at(i).feature.roi.x_offset,
          input_roi_msg->feature_objects.at(i).feature.roi.y_offset +
          input_roi_msg->feature_objects.at(i).feature.roi.height),
        cv::Scalar(0, 255, 0), 3, CV_AA);
      cv::line(
        image,
        cv::Point(
  ...
  try {
    transform_stamped = tf_buffer_.lookupTransform(
      /*target*/ camera_info_ptr_->header.frame_id, /*src*/ input_cluster_msg->header.frame_id,
      ros::Time(0));
  } catch (tf2::TransformException & ex) {
    ROS_WARN("%s", ex.what());
    return;
  ...
  for (size_t i = 0; i < input_cluster_msg->feature_objects.size(); ++i) {
      ...
      ++iter_x, ++iter_y, ++iter_z)
    {
      ...
      if (0 <= static_cast<int>(normalized_projected_point.x()) &&
        static_cast<int>(normalized_projected_point.x()) <=
        static_cast<int>(camera_info_ptr_->width) - 1 &&
        0 <= static_cast<int>(normalized_projected_point.y()) &&
        static_cast<int>(normalized_projected_point.y()) <=
        static_cast<int>(camera_info_ptr_->height) - 1)
      {
        min_x = std::min(static_cast<int>(normalized_projected_point.x()), min_x);
        min_y = std::min(static_cast<int>(normalized_projected_point.y()), min_y);
        max_x = std::max(static_cast<int>(normalized_projected_point.x()), max_x);
  ...
  cv::Mat image(cv::Size(camera_info_ptr_->width, camera_info_ptr_->height), CV_8UC3, cv::Scalar(
      0,
      0,
      0));
      ...
      ++iter_x, ++iter_y, ++iter_z)
    {
        ...
        static_cast<int>(camera_info_ptr_->height) - 1)
      {
        ...
        cv::circle(
          ...
          cv::Point(
            static_cast<int>(normalized_projected_point.x()),
            static_cast<int>(normalized_projected_point.y())), 2, cv::Scalar(255, 255, 255), 3, 4);
      }
    }
    if (projected_points.empty()) {
      continue;
