  ...
  const DebugData & debug_data, const int64_t module_id)
{
  ...
  // Crosswalk polygons
  int32_t uid = static_cast<int32_t>(planning_utils::bitShift(module_id));
  for (size_t i = 0; i < debug_data.crosswalk_polygons.size(); ++i) {
    ...
    marker.id = static_cast<int32_t>(i) + uid;
  ...
  for (size_t i = 0; i < debug_data.collision_lines.size(); ++i) {
    ...
    marker.id = uid + static_cast<int32_t>(i);
    marker.lifetime = rclcpp::Duration::from_seconds(0.5);
    marker.type = visualization_msgs::msg::Marker::LINE_STRIP;
    marker.action = visualization_msgs::msg::Marker::ADD;
    marker.pose.position.x = 0;
  ...
  if (!debug_data.stop_poses.empty()) {
    ...
    marker.id = static_cast<int32_t>(module_id);
    marker.lifetime = rclcpp::Duration::from_seconds(0.5);
    marker.type = visualization_msgs::msg::Marker::POINTS;
    marker.action = visualization_msgs::msg::Marker::ADD;
    marker.pose.position.x = 0;
  ...
  for (size_t j = 0; j < debug_data.stop_poses.size(); ++j) {
    ...
    marker.id = uid + static_cast<int32_t>(j);
    marker.lifetime = rclcpp::Duration::from_seconds(0.5);
  ...
  const DebugData & debug_data, const int64_t module_id)
{
  ...
  for (size_t j = 0; j < debug_data.stop_poses.size(); ++j) {
    ...
    range_marker.id = uid + static_cast<int32_t>(debug_data.stop_poses.size() + j);
