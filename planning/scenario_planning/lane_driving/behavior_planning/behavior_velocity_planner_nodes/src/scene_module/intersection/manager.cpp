  ...
  const lanelet::LaneletMapPtr lanelet_map)
{
  ...
  for (const auto & p : path.points) {
    const uint64_t lane_id = p.lane_ids.at(0);
    const auto lane = lanelet_map->laneletLayer.get(static_cast<int64_t>(lane_id));
...
std::set<int64_t> getLaneIdSetOnPath(const autoware_auto_msgs::msg::PathWithLaneId & path)
{
  std::set<int64_t> lane_id_set;

  for (const auto & p : path.points) {
    for (const auto & lane_id : p.lane_ids) {
      lane_id_set.insert(static_cast<int64_t>(lane_id));
    }
  }
...
//  const auto vehicle_info = vehicle_info_util::VehicleInfoUtil(node).getVehicleInfo();
  ...
  p.stuck_vehicle_ignore_dist = node.declare_parameter(ns + ".stuck_vehicle_ignore_dist", 5.0) +
    vehicle_info.offset_longitudinal_max;
