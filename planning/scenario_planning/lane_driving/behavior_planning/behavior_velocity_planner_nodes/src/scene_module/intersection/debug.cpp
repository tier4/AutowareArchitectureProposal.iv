  ...
  const int64_t lane_id)
{
  ...
  int32_t i = 0;
  int32_t uid = static_cast<int32_t>(planning_utils::bitShift(lane_id));
  for (const auto & polygon : polygons) {
    ...
    marker.color = createMarkerColor(0.0, 1.0, 0.0, 0.999f);
  ...
  const float b)
{
  ...
  marker.ns = ns;
  marker.id = static_cast<int32_t>(lane_id);
  marker.lifetime = rclcpp::Duration::from_seconds(0.3);
  marker.type = visualization_msgs::msg::Marker::LINE_STRIP;
  marker.action = visualization_msgs::msg::Marker::ADD;
  marker.pose.orientation = createMarkerOrientation(0, 0, 0, 1.0);
  ...
  const double b)
{
  ...
  for (const auto & p : path.points) {
    ...
    if (std::find(p.lane_ids.begin(), p.lane_ids.end(), lane_id) != p.lane_ids.end()) {
      ...
      marker.color = createMarkerColor(0.5, 0.5, 0.5, 0.999f);
    }
    msg.markers.push_back(marker);
  }
...
visualization_msgs::msg::MarkerArray createVirtualStopWallMarkerArray(
  const geometry_msgs::msg::Pose & pose, const int32_t lane_id, const std::string & stop_factor)
{
  visualization_msgs::msg::MarkerArray msg;
  ...
  marker_factor_text.color = createMarkerColor(1.0f, 1.0f, 1.0f, 0.999f);
  ...
  const geometry_msgs::msg::Pose & pose, const int32_t lane_id, const std::string & slow_factor)
{
  ...
  marker_factor_text.id = static_cast<int>(lane_id);
  marker_factor_text.lifetime = rclcpp::Duration::from_seconds(0.5);
  marker_factor_text.type = visualization_msgs::msg::Marker::TEXT_VIEW_FACING;
  marker_factor_text.action = visualization_msgs::msg::Marker::ADD;
  marker_factor_text.pose = pose;
  ...
  const float b)
{
  ...
  marker_line.id = static_cast<int>(id);
  ...
  marker_line.color = createMarkerColor(r, g, b, 0.999f);
  ...
  if (state == IntersectionModule::State::STOP) {
    appendMarkerArray(
      createPoseMarkerArray(
        debug_data_.stop_point_pose, "stop_point_pose", lane_id_, 1.0f, 0.0f, 0.0f),
      current_time,
      &debug_marker_array);

    appendMarkerArray(
      createPoseMarkerArray(
        debug_data_.judge_point_pose, "judge_point_pose", lane_id_, 1.0f, 1.0f, 0.5f),
      current_time,
      &debug_marker_array);
