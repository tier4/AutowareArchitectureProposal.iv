...
BehaviorModuleOutput PullOverModule::plan()
{
  ...
  const auto hazard_info = getHazard(
    ...
    planner_data_->route_handler->getGoalPose(), planner_data_->self_velocity->twist.linear.x,
  ...
  const auto turn_info = util::getPathTurnSignal(
    ...
    planner_data_->self_pose->pose, planner_data_->self_velocity->twist.linear.x,
    planner_data_->parameters, parameters_.pull_over_search_distance);

  if (hazard_info.first.data == TurnSignal::HAZARD) {
    output.turn_signal_info.turn_signal.data = hazard_info.first.data;
    output.turn_signal_info.signal_distance = hazard_info.second;
  } else {
    output.turn_signal_info.turn_signal.data = turn_info.first.data;
...
std::pair<TurnSignal, double> PullOverModule::getHazard(
  ...
  const double & hazard_on_threshold_vel, const double & base_link2front) const
{
  ...
  if (
    ...
    return std::make_pair(turn_signal, distance_to_target_point);
  }
