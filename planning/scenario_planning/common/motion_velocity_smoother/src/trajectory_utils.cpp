  ...
  const TrajectoryPointArray & trajectory, const Pose & target_pose)
{
  ...
  auto v1 = getTransVector3(
    trajectory.at(segment_idx).pose, trajectory.at(segment_idx + 1).pose);
...
boost::optional<TrajectoryPointArray> extractPathAroundIndex(
  const TrajectoryPointArray & trajectory, const size_t index,
  const double & ahead_length, const double & behind_length)
{
  ...
  size_t ahead_index{trajectory.size() - 1};
  {
    ...
    for (size_t i = index; i < trajectory.size() - 1; ++i) {
      dist_sum +=
        autoware_utils::calcDistance2d(trajectory.at(i), trajectory.at(i + 1));
...
double calcArcLength(
  const TrajectoryPointArray & path, const int idx1, const int idx2)
{
...
std::vector<double> calcTrajectoryIntervalDistance(
  const TrajectoryPointArray & trajectory)
{
...
double getMaxVelocity(const TrajectoryPointArray & trajectory)
{
  double max_vel = 0.0;
  for (auto & tp : trajectory) {
    if (tp.longitudinal_velocity_mps > max_vel) {max_vel = tp.longitudinal_velocity_mps;}
  }
  return max_vel;
}
...
boost::optional<TrajectoryPointArray> applyLinearInterpolation(
  ...
  const TrajectoryPointArray & base_trajectory, const std::vector<double> & out_index,
...
boost::optional<TrajectoryPointArray> applyDecelFilterWithJerkConstraint(
  const TrajectoryPointArray & input, const size_t start_index, const double v0,
