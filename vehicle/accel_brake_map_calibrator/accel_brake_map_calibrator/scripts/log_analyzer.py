...
parser.add_argument('--min-vel-thr', type=float,
                    default=0.1, help='thresh of minimum velocity')
parser.add_argument('--max-pedal-vel-thr', type=float,
                    default=0.15, help='thresh of maximum pedal velocity')
parser.add_argument('--max-steer-thr', type=float,
                    default=0.2, help='thresh of maximum steer')
parser.add_argument('--max-pitch-thr', type=float,
                    default=0.02, help='thresh of maximum pitch')
parser.add_argument('--max-jerk-thr', type=float,
                    default=0.7, help='thresh of maximum jerk')
parser.add_argument('--pedal-diff-thr', type=float,
                    default=0.03,
                    help='thresh of max delta-pedal to add statistics map')
parser.add_argument('--vel-diff-thr', type=float,
                    default=0.556,
                    help='thresh of max delta-velocity to add statistics map')
parser.add_argument('--view-velocity-1', type=int,
                    default=5,
                    help='velocity(kmh) to visualize by plot (1)')
parser.add_argument('--view-velocity-2', type=int,
                    default=20,
                    help='velocity(kmh) to visualize by plot (2)')
parser.add_argument('--view-velocity-3', type=int,
                    default=30,
                    help='velocity(kmh) to visualize by plot (3)')
parser.add_argument('--output-stat-csv',
                    default='stat.csv',
...
for val in CONSIDER_PEDAL_VAL_FOR_STAT:
    if not(val in CF.PEDAL_LIST):
        print('invalid CONSIDER_PEDAL_VAL_FOR_STAT. pedal list is:',
              CF.PEDAL_LIST)
...
data, full_data = CalcUtils.create_2d_map(vel_data, pedal_data, acc_data, color_data,
...
plotter = Plotter(2, 3)
plotter.subplot(1)
plotter.imshow(average_map, CF.VEL_MIN, CF.VEL_MAX, CF.VEL_SPAN,
               CF.PEDAL_MIN, CF.PEDAL_MAX, CF.PEDAL_SPAN)
plotter.add_label('average of accel', 'velocity(kmh)', 'throttle')

plotter.subplot(2)
plotter.imshow(stddev_map, CF.VEL_MIN, CF.VEL_MAX, CF.VEL_SPAN,
               CF.PEDAL_MIN, CF.PEDAL_MAX, CF.PEDAL_SPAN)
plotter.add_label('std. dev. of accel', 'velocity(kmh)', 'throttle')

plotter.subplot(3)
plotter.imshow(count_map, CF.VEL_MIN, CF.VEL_MAX, CF.VEL_SPAN,
               CF.PEDAL_MIN, CF.PEDAL_MAX, CF.PEDAL_SPAN, num_data_type='int')
...
def view_pedal_accel_graph(subplot_num, vel_list_idx):
    ...
    plotter.scatter(velocity_map_list[vel_list_idx]
    ...
    plotter.scatter(
        CF.PEDAL_LIST, average_map_v_avoid_0,
        'red', label='average')

    plotter.set_lim(fig, SCATTER_GRAPH_PEDAL_LIM, SCATTER_GRAPH_ACCEL_LIM)
    plotter.add_label(str(view_velocity_list[vel_list_idx]) +
