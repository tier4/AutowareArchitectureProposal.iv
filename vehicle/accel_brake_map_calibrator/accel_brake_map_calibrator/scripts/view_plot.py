...
class ViewPlot(Node):
    ...
    def __init__(self, args):
        ...
        if default_map_dir is None:
            ...
            self.declare_parameter('/accel_brake_map_calibrator/csv_default_map_dir',
                                   package_path + '/data/default/')
            default_map_dir = self.get_parameter(
                '/accel_brake_map_calibrator/csv_default_map_dir'
            ).get_parameter_value().string_value

        if calibrated_map_dir is None:
            package_path = get_package_share_directory('accel_brake_map_calibrator')
            self.declare_parameter('/accel_brake_map_calibrator/csv_calibrated_map_dir',
                                   package_path + '/config/')
            calibrated_map_dir = self.get_parameter(
                '/accel_brake_map_calibrator/csv_calibrated_map_dir'
            ).get_parameter_value().string_value
        ...
        data, full_data = CalcUtils.create_2d_map(
            vel_data, pedal_data, acc_data, color_data,
            CF.VEL_LIST/3.6, vel_diff_thr,
            CF.PEDAL_LIST, pedal_diff_thr)

        count_map, average_map, stddev_map = CalcUtils.create_stat_map(data)
        velocity_map_list = []
        for i in range(len(CF.VEL_LIST)):
            velocity_map_list.append(
                CalcUtils.extract_x_index_map(full_data, i))
        ...
        if len(default_pedal_list) == 0 or len(default_acc_list) == 0:
            self.get_logger().warning(
                'No default map file was found in {}'.format(default_map_dir))
        ...
        for i in range(len(CF.VEL_LIST)):
            self.view_pedal_accel_graph(
                plotter, i,
                velocity_map_list, i,
                count_map, average_map, stddev_map,
                default_pedal_list, default_acc_list,
                calibrated_pedal_list, calibrated_acc_list, scatter_only)
        plotter.show()

    def plotter_function(self):
        return self.plotter

    def view_pedal_accel_graph(self, plotter, subplot_num,
                               velocity_map_list, vel_list_idx,
                               count_map, average_map, stddev_map,
                               default_pedal_list, default_acc_list,
                               calibrated_pedal_list, calibrated_acc_list, scatter_only):
        ...
        if not scatter_only:
            ...
            if len(calibrated_pedal_list) != 0 and len(calibrated_acc_list) != 0:
                plotter.plot(calibrated_pedal_list[vel_list_idx],
                             calibrated_acc_list[vel_list_idx],
                             color='blue', label='calibrated')

            # default map
            if len(default_pedal_list) != 0 and len(default_acc_list) != 0:
                plotter.plot(default_pedal_list[vel_list_idx],
                             default_acc_list[vel_list_idx],
                             color='orange', label='default', linestyle='dashed')

        # plot all data
        pedal_list = [0 for i in range(len(CF.PEDAL_LIST))]
        if(velocity_map_list[vel_list_idx] is not None):
            plotter.scatter_color(velocity_map_list[vel_list_idx]
                                  [:, 1], velocity_map_list[vel_list_idx][:, 2],
                                  color=velocity_map_list[vel_list_idx][:, 3], label='all')
        ...
        # plot average data
        plotter.scatter(
            CF.PEDAL_LIST, average_map[:, vel_list_idx],
            'red', label='average')
        ...
        for i in range(len(CF.PEDAL_LIST)):
            ...
            plotter.plot_text(x, y-2, '{}\npts'.format(pedal_list[i]),
                              num_data_type='str', color='green')

        pedal_lim = [CF.PEDAL_LIST[0] - 0.05, CF.PEDAL_LIST[-1] + 0.05]
        accel_lim = [-5.0, 5.0]

        plotter.set_lim(fig, pedal_lim, accel_lim)
        plotter.add_label(str(CF.VEL_LIST[vel_list_idx]) +
                          'kmh; pedal-accel relation', 'pedal', 'accel')
    ...
    def load_map(self, csv_dir):
        try:
            ...
            return np.hstack([np.fliplr(np.array(accel_pedal_list).T),
                              np.array(brake_pedal_list).T.tolist()]), \
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--default-map-dir', default=None,
                        type=str, help='directory of default map')
    parser.add_argument('-c', '--calibrated-map-dir', default=None,
                        type=str, help='directory of calibrated map')
    parser.add_argument('-s', '--scatter-only', action='store_true',
                        help='show only scatters')
    parser.add_argument('-l', '--log-file', default=package_path + '/config/log.csv',
                        type=str, help='path of log.csv')
    parser.add_argument('--min-vel-thr', default=0.1, type=float,
                        help='valid min velocity threshold')
    parser.add_argument('--vel-diff-thr', default=0.556, type=float,
                        help='valid velocity diff threshold')
    parser.add_argument('--pedal-diff-thr', default=0.03, type=float,
                        help='valid pedal diff threshold')
    parser.add_argument('--max-steer-thr', default=0.2, type=float,
                        help='valid max steer threshold')
    parser.add_argument('--max-pitch-thr', default=0.02, type=float,
                        help='valid max pitch threshold')
    parser.add_argument('--max-jerk-thr', default=0.7, type=float,
                        help='valid max jerk threshold')
    parser.add_argument('--max-pedal-vel-thr', default=0.7, type=float,
                        help='valid max pedal velocity threshold')
