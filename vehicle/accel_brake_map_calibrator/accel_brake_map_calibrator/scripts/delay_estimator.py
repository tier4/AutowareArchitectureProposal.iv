...
class DelayEstimator(Node):
    ...
    def __init__(self):
        ...
        min_vel_thr = self.get_parameter(
            'velocity_min_threshold').get_parameter_value().double_value
        self.declare_parameter('velocity_diff_threshold', 0.556)
        vel_diff_thr = self.get_parameter(
            'velocity_diff_threshold').get_parameter_value().double_value
        self.declare_parameter('pedal_diff_threshold', 0.03)
        pedal_diff_thr = self.get_parameter(
            'pedal_diff_threshold').get_parameter_value().double_value
        self.declare_parameter('max_steer_threshold', 0.2)
        max_steer_thr = self.get_parameter(
            'max_steer_threshold').get_parameter_value().double_value
        self.declare_parameter('max_pitch_threshold', 0.02)
        max_pitch_thr = self.get_parameter(
            'max_pitch_threshold').get_parameter_value().double_value
        self.declare_parameter('max_jerk_threshold', 0.7)
        max_jerk_thr = self.get_parameter(
            'max_jerk_threshold').get_parameter_value().double_value
        self.declare_parameter('pedal_velocity_thresh', 0.15)
        max_pedal_vel_thr = self.get_parameter('pedal_velocity_thresh'
        ...
        for delay_step in range(max_delay_step + 1):
            ...
            csv_data = \
                self.cr.extractPedalRangeWithDelay(
            ...
            for vel_value in VEL_VALUE_LIST:
                ex_csv_data = csv_data[csv_data[CF.VEL]
                                       < vel_value + vel_diff_thr]
                ex_csv_data = csv_data[csv_data[CF.VEL]
                                       > vel_value - vel_diff_thr]
                pedal_speed = ex_csv_data[CF.A_PED_SPD] - \
                    ex_csv_data[CF.B_PED_SPD]
            ...
            self.plotPedalSpeedAndAccel(
                csv_data, plotter, delay_step + 1, delay_step, ave_coef)
        plotter.show()
    ...
    def plotPedalSpeedAndAccel(self, csv_data, plotter, subplot_num, delay_step, coef):
        ...
        plotter.add_label(
            'pedal-spd-acc (delay = ' +
            str(delay_time_ms) + ' ms), R = ' + str(coef),
            'pedal-speed', 'accel')
        plotter.set_lim(fig,
                        [-0.4, 0.4],
                        [-1.0, 1.0])
