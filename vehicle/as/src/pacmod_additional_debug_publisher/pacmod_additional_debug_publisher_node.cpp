...
{
  debug_pub_ = nh_.advertise<std_msgs::Float32MultiArray>("output/debug", 1);
  sub_ = nh_.subscribe("input/can_tx", 1, &PacmodAdditionalDebugPublisherNode::canTxCallback, this);
  debug_value_.data.resize(17);
  pnh_.param<bool>("calibration_active", calibration_active_, false);
  if (calibration_active_) {
    accel_cal_rpt_pub_ = nh_.advertise<std_msgs::Float32MultiArray>("output/accel_cal_rpt", 1);
    brake_cal_rpt_pub_ = nh_.advertise<std_msgs::Float32MultiArray>("output/brake_cal_rpt", 1);
    steer_cal_rpt_pub_ = nh_.advertise<std_msgs::Float32MultiArray>("output/steer_cal_rpt", 1);
