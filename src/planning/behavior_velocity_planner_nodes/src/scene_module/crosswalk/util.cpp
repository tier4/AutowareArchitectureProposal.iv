...
bool insertTargetVelocityPoint(
  const autoware_auto_msgs::msg::PathWithLaneId & input, const Polygon & polygon,
  const double & margin, const double & velocity, const PlannerData & planner_data,
  autoware_auto_msgs::msg::PathWithLaneId & output, DebugData & debug_data,
  boost::optional<int> & first_stop_path_point_index)
{
  ...
  for (size_t i = 0; i < output.points.size() - 1; ++i) {
    ...
    const double base_link2front = planner_data.vehicle_constants_.offset_longitudinal_max;
    double length_sum = 0;
    ...
    autoware_auto_msgs::msg::PathPointWithLaneId target_point_with_lane_id;
...
bool insertTargetVelocityPoint(
  const autoware_auto_msgs::msg::PathWithLaneId & input,
  const lanelet::ConstLineString3d & stop_line, const double & margin, const double & velocity,
  const PlannerData & planner_data, autoware_auto_msgs::msg::PathWithLaneId & output,
