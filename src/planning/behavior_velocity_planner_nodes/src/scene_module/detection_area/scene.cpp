...
std::pair<int, double> findWayPointAndDistance(
  const autoware_auto_msgs::msg::PathWithLaneId & path, const geometry_msgs::msg::Point & p)
{
...
double calcArcLengthFromWayPoint(
  const autoware_auto_msgs::msg::PathWithLaneId & path, const int & src, const int & dst)
{
  double length = 0;
...
double calcSignedArcLength(
  const autoware_auto_msgs::msg::PathWithLaneId & path, const geometry_msgs::msg::Point & p1,
  ...
  const LineString2d & stop_line, const LineString2d & path_segment)
{
  ...
  const long min_idx = std::distance(dist_list.cbegin(), min_itr);

  return collision_points.at(static_cast<size_t>(min_idx));
}
...
boost::optional<PathIndexWithOffset> findForwardOffsetSegment(
  const autoware_auto_msgs::msg::PathWithLaneId & path, const size_t base_idx,
  ...
  const double offset_length)
{
  ...
  for (std::int32_t i = start; i >= 0; --i) {
    const auto p_front = to_bg2d(path.points.at(static_cast<size_t>(i)).point.pose.position);
    const auto p_back = to_bg2d(path.points.at(static_cast<size_t>(i + 1)).point.pose.position);
...
autoware_auto_msgs::msg::PathWithLaneId DetectionAreaModule::insertStopPoint(
  const autoware_auto_msgs::msg::PathWithLaneId & path,
  const PathIndexWithPose & stop_point) const
{
  ...
  autoware_auto_msgs::msg::PathPointWithLaneId stop_point_with_lane_id;
