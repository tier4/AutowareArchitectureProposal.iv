  ...
  const double offset_length)
{
  ...
  const auto start = base_idx - 1;
  for (auto i = static_cast<int32_t>(start); i >= 0; --i) {
    const auto index = static_cast<size_t>(i);
    const auto p_front = to_bg2d(path.points.at(index).point.pose.position);
    const auto p_back = to_bg2d(path.points.at(index + 1).point.pose.position);
  ...
  const autoware_auto_msgs::msg::PathWithLaneId & path, const geometry_msgs::msg::Point & point)
{
  ...
  std::transform(
    ...
    [&point](const autoware_auto_msgs::msg::PathPointWithLaneId & p) {
...
double calcSignedArcLength(
  const autoware_auto_msgs::msg::PathWithLaneId & path, const size_t & idx_front,
...
autoware_auto_msgs::msg::PathWithLaneId StopLineModule::insertStopPose(
  const autoware_auto_msgs::msg::PathWithLaneId & path,
