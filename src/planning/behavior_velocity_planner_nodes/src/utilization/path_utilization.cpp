...
{
  autoware_auto_msgs::msg::Path interpolated_path;
    ...
    s_t += interpolation_interval)
  {
    ...
    while (checkpoint_idx < spline_ptr->s.size() && spline_ptr->s.at(checkpoint_idx) < s_t) {
      autoware_auto_msgs::msg::PathPoint path_point;
      ...
      try {
        path_point.is_final = path.points.at(checkpoint_idx).is_final;
...
{
  autoware_auto_msgs::msg::Path filtered_path;
...
autoware_auto_msgs::msg::Path filterStopPathPoint(
  const autoware_auto_msgs::msg::Path & path)
{
  autoware_auto_msgs::msg::Path filtered_path = path;
