...
{
auto jsonDumpsPose = [](const geometry_msgs::msg::Pose & pose) {
...
autoware_auto_msgs::msg::PathWithLaneId BehaviorVelocityPlannerManager::planPathVelocity(
  const std::shared_ptr<const PlannerData> & planner_data,
  const autoware_auto_msgs::msg::PathWithLaneId & input_path_msg)
{
  autoware_auto_msgs::msg::PathWithLaneId output_path_msg = input_path_msg;
  ...
  stop_reason_diag_ = makeStopReasonDiag(
    stop_reason_msg,
    output_path_msg.points[static_cast<size_t>(first_stop_path_point_index)].point.pose);
