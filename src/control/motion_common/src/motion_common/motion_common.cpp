...
{
  const auto w = pt.heading.real;
  const auto z = pt.heading.imag;
  // double angle rule
...
{
  const auto nx = next_pt.x - current_pt.x;
  const auto ny = next_pt.y - current_pt.y;
...
bool is_past_point(
  ...
  const Point & pt,
  const Real nx,
  const Real ny) noexcept
{
  const auto dx = (state.x - pt.x);
  const auto dy = (state.y - pt.y);

  // Check if state is past last_pt when projected onto the ray defined by heading
  return ((nx * dx) + (ny * dy)) >= -std::numeric_limits<Real>::epsilon();
}
...
bool heading_ok(const Trajectory & traj)
{
  const auto bad_heading = [](const auto & pt) -> bool {
      const auto real2 = pt.heading.real * pt.heading.real;
      const auto imag2 = pt.heading.imag * pt.heading.imag;
...
{
  using tfReal = decltype(transform.transform.translation.x);
  t_out = t_in;
  // TODO(c.ho) normalize quaternion
  // Compute rotation matrix
  const auto & q = transform.transform.rotation;
  const auto r2 = q.w * q.w;
  const auto i2 = q.x * q.x;
  const auto j2 = q.y * q.y;
  const auto k2 = q.z * q.z;
  {
    const auto s = 1.0 / (r2 + i2 + j2 + k2);

    const auto a11 = 1.0 - (2.0 * s * (j2 + k2));
    const auto a12 = 2.0 * s * ((q.x * q.y) - (q.z * q.w));

    const auto a21 = 2.0 * s * ((q.x * q.y) + (q.z * q.w));
    const auto a22 = 1.0 - (2.0 * s * (i2 + k2));
    // Apply rotation
    const auto x = static_cast<tfReal>(t_in.x);
    const auto y = static_cast<tfReal>(t_in.y);
    t_out.x = static_cast<Real>((x * a11) + (y * a12));
    t_out.y = static_cast<Real>((x * a21) + (y * a22));
  }
  {
    // TODO(c.ho) should I be doing the full 3d rotation...?
    const auto s = 1.0 / std::sqrt(r2 + k2);
    const auto w = q.w * s;
    const auto z = q.z * s;
    const auto real = static_cast<tfReal>(t_in.heading.real);
    const auto imag = static_cast<tfReal>(t_in.heading.imag);
    t_out.heading.real = static_cast<Real>((real * w) - (imag * z));
    t_out.heading.imag = static_cast<Real>((real * z) + (imag * w));
  }
  // Apply translation
  t_out.x += static_cast<Real>(transform.transform.translation.x);
  t_out.y += static_cast<Real>(transform.transform.translation.y);
}
...
{
  {
    ...
    const auto q = ref.heading;
    const auto nx = (q.real * q.real) - (q.imag * q.imag);
    const auto ny = decltype(nx) {2.0} *q.real * q.imag;
    // project state onto reference basis
    const auto dx = state.x - ref.x;
    const auto dy = state.y - ref.y;
    // normals rotated +90 deg
    out.lateral_error_m = (dx * (-ny)) + (dy * nx);
    out.longitudinal_error_m = (dx * nx) + (dy * ny);
  }
  ...
  out.yaw_error_rad = to_angle(state.heading - ref.heading);
