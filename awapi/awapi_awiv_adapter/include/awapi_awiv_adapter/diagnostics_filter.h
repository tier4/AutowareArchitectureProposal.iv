...
inline bool isChild(
  const diagnostic_msgs::DiagnosticStatus & child, const diagnostic_msgs::DiagnosticStatus & parent)
{
...
inline bool isLeaf(
  ...
  const diagnostic_msgs::DiagnosticStatus & diag)
{
  return diag_name_set.count(diag.name) == 0;
}

inline std::unordered_set<std::string> createDiagNameSet(
  const std::vector<diagnostic_msgs::DiagnosticStatus> & diagnostics)
{
...
inline std::vector<diagnostic_msgs::DiagnosticStatus> extractLeafDiagnostics(
  const std::vector<diagnostic_msgs::DiagnosticStatus> & diagnostics)
{
  const auto diag_name_set = createDiagNameSet(diagnostics);

  std::vector<diagnostic_msgs::DiagnosticStatus> leaf_diagnostics;
  for (const auto & diag : diagnostics) {
...
inline std::vector<diagnostic_msgs::DiagnosticStatus> extractLeafChildrenDiagnostics(
  const diagnostic_msgs::DiagnosticStatus & parent,
  const std::vector<diagnostic_msgs::DiagnosticStatus> & diagnostics)
{
  std::vector<diagnostic_msgs::DiagnosticStatus> leaf_children_diagnostics;
